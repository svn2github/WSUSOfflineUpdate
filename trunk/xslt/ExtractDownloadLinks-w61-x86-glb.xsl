...
<xsl:template match="*">
  <xsl:choose>
    <xsl:when test="name()='FileLocation'">
      ...
      <xsl:if test="contains(@Url, 'http://') and (contains(@Url, '/ie8') or contains(@Url, '/ie9') or contains(@Url, '/ie10')) and contains(@Url, 'windows6.1') and contains(@Url, '-x86') and contains(@Url, '.cab')">
        <xsl:value-of select="@Url"/>
        <xsl:text>&#10;</xsl:text>
      </xsl:if>
    </xsl:when>
