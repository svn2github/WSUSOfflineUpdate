...
for %%i in (w62 w62-x64 w63 w63-x64) do (
  if /i "%~2"=="%%i" (
...
echo Usage: %~n0 ^<SxsPath^> {w62 ^| w62-x64 ^| w63 ^| w63-x64} {enu ^| fra ^| esn ^| jpn ^| kor ^| rus ^| ptg ^| ptb ^| deu ^| nld ^| ita ^| chs ^| cht ^| plk ^| hun ^| csy ^| sve ^| trk ^| ell ^| ara ^| heb ^| dan ^| nor ^| fin} [/cleanup]
